...
By contributing to Configurable Splash Texts, you agree with the [Developer Certificate of Origin (DCO)][DCO].
...
Use American English to keep consistency across Configurable Splash Texts.

If there is more than one acceptable spelling of the same word, first check if one word is already
being used in Fabric API or in Mojang's Mappings, and if not, use the spelling that is most commonly used.
...
It allows to separate the actual name of the variable and the namespace, `configurableSplashTexts`.

Fields marked as `@Unique` must be prefixed with `configurableSplashTexts$` or `CONFIGURABLESPLASHTEXTS$` if the field is static and final.
Methods marked with `@Unique` do not need prefixes.


## Licensing & DCO

Configurable Splash Texts is licensed under [LGPL 3.0][LICENSE], and it has a [Developer Certificate of Origin (DCO)][DCO], which you are required to agree with to contribute.
...
[LICENSE]: ./LICENSE "Configurable Splash Texts license file"
